...
class ViewController: UIViewController {
    ...
    lazy var mapView: MKMapView = {                     // use lazy to avoid reinizialization more times
        let map = MKMapView()
        map.delegate = self
    ...
    lazy var searchTextField: UITextField = {
        ...
        searchText.leftView = UIView(frame: CGRect(x: 0, y: 0, width: 10, height: 0))       // padding
        searchText.leftViewMode = .always                                                   // always display padding
    ...
    override func viewDidLoad() {
        ...
        locationManager?.delegate = self            // viewController will become the delegate of all diffierent functions location can call
    ...
    private func findNearbyPlaces(by query: String){
        ...
        search.start { [weak self] response, error in
            ...
            self?.places = response.mapItems.map(PlaceAnnotation.init)            // pass all item inside PlaceAnnotation constructor
